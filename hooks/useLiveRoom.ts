...
export function useUnifiedTipping(chainId: number) {
  ...
  const getContractInstance = useCallback(() => {
    ...
    if (!contractAddress) {
      throw new Error("Missing unifiedTipping contract address for current chain");
    }
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes("Missing unifiedTipping contract address")) {
